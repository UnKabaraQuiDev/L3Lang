	...
	protected void accept(X86Compiler compiler, MemoryStatus mem, ScopeContainer container, FieldAccessNode node) throws CompilerException {
		...
		if (node.getIdent().size() > 1) {
			...
			if (node.hasFunDefParent() && (funDef = node.getFunDefParent()).isParamDefDescriptor(ident)) { // fun param
				...
				compiler.writeinstln("mov " + mem.getAsSize(reg, size) + ", [rbp+" + (8 + (paramsSize - def.getStackOffset())) + "]  ; Load param: " + letDef.getIdent()+" > "+def.getStackOffset()+"/"+paramsSize);
				...
				if (letDef.isiStatic()) {
					compiler.writeinstln("mov " + mem.getAsSize(reg, size) + ", [" + def.getAsmName() + "]  ; Load static var: " + letDef.getIdent());
				} else {
					compiler.writeinstln("mov " + mem.getAsSize(reg, size) + ", [rbp-" + (size + def.getStackOffset()) + "]  ; Load local var: " + letDef.getIdent());
				}
